...
public final class Yen implements KSPAlgorithm {
    ...
    public List<PathUtil> ksp(Graph graph, String sourceLabel, String targetLabel, int K) {
        ...
        PriorityQueue<PathUtil> candidates = new PriorityQueue<PathUtil>();

        try {
            /* Compute and add the shortest path */
            PathUtil kthPath = Dijkstra.shortestPath(graph, sourceLabel, targetLabel);
            ksp.add(kthPath);

            /* Iteratively compute each of the k shortest paths */
            for (int k = 1; k < K; k++) {
                // Get the (k-1)st shortest path
                PathUtil previousPath = ksp.get(k-1);
                ...
                for (int i = 0; i < previousPath.size(); i++) {
                    ...
                    PathUtil rootPath = previousPath.cloneTo(i);

                    /* Iterate over all of the (k-1) shortest paths */
                    for(PathUtil p:ksp) {
                        PathUtil stub = p.cloneTo(i);
                    ...
                    PathUtil spurPath = Dijkstra.shortestPath(graph, spurNode, targetLabel);

                    // If a new spur path was identified...
                    if (spurPath != null) {
                        // Concatenate the root and spur paths to form the new candidate path
                        PathUtil totalPath = rootPath.clone();
     ...
     * @return              a list of the K shortest paths from s to t, ordered from shortest to longest
     */
    public static  List<PathUtil> ksp_v2(Graph graph, String sourceLabel, String targetLabel, int K) {
        // Initialize containers for candidate paths and k shortest paths
        ArrayList<PathUtil> ksp = new ArrayList<PathUtil>();
        PriorityQueue<PathUtil> candidates = new PriorityQueue<PathUtil>();
