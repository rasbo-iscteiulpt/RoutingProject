 ...
 */
public class PathUtil implements Cloneable, Comparable<PathUtil> {
    ...
    public PathUtil(double totalCost) {
        edges = new LinkedList<Edge>();
        this.totalCost = totalCost;
    }

    public PathUtil(LinkedList<Edge> edges) {
        this.edges = edges;
        totalCost = 0;
        for (Edge edge : edges) {
    ...
    public PathUtil(LinkedList<Edge> edges, double totalCost) {
        this.edges = edges;
        this.totalCost = totalCost;
    }
    ...
    public String toString() {
        ...
        sb.append("Num Edges: " + numEdges);
        sb.append(" -> ");
        int nPOI = this.nPOIs();
        sb.append("Num POIs: " + nPOI);
        sb.append(" -> ");
        sb.append(totalCost);
        sb.append(": [");
        if (numEdges > 0) {
...
/*    @Override
    ...
    public boolean equals(PathUtil path2) {
        if (path2 == null)
            return false;
    ...
    public int compareTo(PathUtil path2) {
    ...
    public PathUtil clone() {
        ...
        return new PathUtil(edges);
    }

    public PathUtil shallowClone() {
        LinkedList<Edge> edges = new LinkedList<Edge>();
        ...
        return new PathUtil(edges,this.totalCost);
    }

    public PathUtil cloneTo(int i) {
        LinkedList<Edge> edges = new LinkedList<Edge>();
    ...
    public PathUtil cloneFrom(int i) {
        LinkedList<Edge> edges = new LinkedList<Edge>();
    ...
    public void addPath(PathUtil p2) {
    ...
    //so aplicavel no caso de ser com o custo sust
    //rita
    public int nPOIs() {
    	int nPOI = 0;
    	
    	for(Edge e: edges) {
    		double sust = e.getWeight();
    		if(sust<200 && sust >=100) {
    			nPOI = nPOI + 1;
    		}
    		else {
    			if(sust<100 && sust >0) { 
